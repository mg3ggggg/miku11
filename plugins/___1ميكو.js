...
const handler = async (m, { conn, text, command, args, usedPrefix }) => {
    const idioma = global.db.data.users[m.sender].language || 'es'; // تحديد لغة المستخدم، إفتراضياً الإسبانية
...
async function simitalk(ask, apikeyyy = "iJ6FxuA9vxlvz5cKQCt3", language = "es") {
    ...
    try {
        ...
        if (trad1.text == 'indefinida' || response1 == '' || !response1.data) trad1 = XD; // تعامل مع حالات خاصة للردود غير المتوقعة
